...
export async function buscarUsuarioPorEmail(email: string): Promise<UsuarioDB | undefined> {
  const db = await connectDB();
  const usuario = await db.get<UsuarioDB>("SELECT * FROM usuarios WHERE email = ?", [email]);
  return usuario;
}
